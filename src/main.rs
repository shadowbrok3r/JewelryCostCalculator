...
async fn main() -> eframe::Result<()> {
    // Load runtime config (SURREAL_URL, SURREAL_USER/PASS) from .env if present.
    dotenvy::dotenv().ok();

    // Headless batch scan: `jewelry_cost_calculator scan <DIR> ...`; otherwise launch the GUI.
    if let Some(code) = scan::dispatch().await {
